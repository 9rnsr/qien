...
/// 
AstNode parseProg(string fname)
{
	scope ctxt = new ParseContext(fname);
	...
	{
		std.stdio.writefln("========");
		std.stdio.writefln("parse = %s", ast.toString);
	}
...
class ParseContext
{
	...
	AstNode parseProg()
	{
		...
		if (t != Token.EOF)
			error(t.pos, "Program should be terminated by EOF");
...
private:
	...
	AstNode parseFun()
	{
		...
		if (t == Token.LPAR || t == Token.LBRAC)
		{
			...
			if (t == Token.LPAR)
			{
				...
				if (t != Token.RPAR)
					error(t.pos, "fun: params RPAR does not close");
				input_next_ws();
			...
			// 引数リストも関数本体もない場合はエラー
			if (!prm && !blk)
				error(t.pos, "fun: missing function param and body");
	...
	AstNode parseCall()
	{
		...
		if (auto n = parsePrimary())
		{
			...
			if (t == Token.LPAR)
			{
				...
				trace.opCall("}Call, t=%s%s", t, t.pos);
				if( t != Token.RPAR )
					error(t.pos, "CallExpr ')' is not closed");
				input_next();
	...
	AstNode parseFactor()
	{
		...
		if (auto n = parseCall())
		{
			...
			if (t == Token.MUL || t == Token.DIV)
			{
				...
				if (!rhs)
					error(t.pos, "factor rhs does not exist");
	...
	AstNode parseTerm()
	{
		if (auto n = parseFactor())
		{
			if (t == Token.ADD || t == Token.SUB)
			{
				...
				auto rhs = parseTerm();
				if (!rhs)
					error(t.pos, "term rhs does not exist");
	...
	AstNode parseAssignExpr()
	{
		...
		if (auto n = parseTerm())
		{
			...
			if (t == Token.ASSIGN)
			{
				...
				auto rhs = parseTerm();
				if (!rhs)
					error(rhs_pos, "AssignExpr rhs does not exist");
	...
	AstNode parseDef()
	{
		...
		else if (t == Token.FUN)
		{
			...
			auto idt = parseIdent();
			if (!idt)
				error(t.pos, "def: ident does not exist");
			trace.opCall("DefFun2, t=%s%s", t, t.pos);
			
			if (t != Token.ASSIGN)
				error(t.pos, "def: initializer does not exist[1]");
			input_next_ws();
			trace.opCall("DefFun3, t=%s%s", t, t.pos);
			
			auto fun = parseFun();
			if (!fun)
				error(t.pos, "def: initializer does not exist[2]");
	...
	AstNode parseBlock()
	{
		...
		if (t == Token.LBRAC)
		{
			...
			if (t != Token.RBRAC)
				error(t.pos, "Block stmt should be terminated by '}'");
			input_next();
	...
	AstNode parseStmt()
	{
		...
		AstNode parseS()
		{
			...
			{
				if (t != Token.NEWLINE)
					error(t.pos, "Stmt should be terminated by NewLine");
				return n;
			}
			if (auto n = parseExpr())
			{
				trace.opCall("parseStmt/parseExpr t=%s%s", t.pos, t);
				if (t != Token.NEWLINE && t != Token.RBRAC)
					error(t.pos, "Stmt should be terminated by NewLine/}");
				return n;
			}
		...
		if (auto n = parseS())
		{
		//	if (t != Token.RBRAC && t != Token.NEWLINE)
		//		error(t.pos, "Stmt should be terminated by NewLine");
		//	input_next_ws();
			input_skip_ws();
	...
	AstNode parseCommaList(scope AstNode delegate() parseX)
	{
		if (auto n = parseX())
		{
			...
			while (t == Token.COMMA)
			{
				...
				tail.next = parseX();
				if (!tail.next)
					error(t.pos, "Comma separated error");
				tail = (tail.next);
			}
			return n;
		}
