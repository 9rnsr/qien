...
class Exp
{
	...
	//	"FLON",	double,
		"VFUN",	Exp, Label,
	//	"FUNC",	Exp, Label,	->	Labelのみ、暗黙にFPを取る
		"CLOS",	Temp,	//	->	Labelのみ、暗黙にpusheしたFPを取る
		"NAME",	Label,
		"TEMP",	Temp,
		"BIN", 	BinOp, Exp, Exp,
...
public:
	void debugOut(TreeOut tout){
		...
		case Tag.CLOS:	return tout(tagname, data2.tupleof[1..$]);
		case Tag.NAME:	return tout(tagname, data3.tupleof[1..$]);
		case Tag.TEMP:	return tout(tagname, data4.tupleof[1..$]);
		case Tag.BIN:	return tout(tagname, data5.tupleof[1..$]);
		case Tag.MEM:	return tout(tagname, data6.tupleof[1..$]);
		case Tag.CALL:	return tout(tagname, data7.tupleof[1..$]);
		case Tag.ESEQ:	return tout(tagname, data8.tupleof[1..$]);
		}
	}
	...
	override bool opEquals(Object o)
	{
		if( auto e = cast(Exp)o ){
			...
			case Tag.CLOS:	foreach( i,t; data2.tupleof ) if( data2.tupleof[i] != e.data2.tupleof[i] ) return false;		break;
			case Tag.NAME:	foreach( i,t; data3.tupleof ) if( data3.tupleof[i] != e.data3.tupleof[i] ) return false;		break;
			case Tag.TEMP:	foreach( i,t; data4.tupleof ) if( data4.tupleof[i] != e.data4.tupleof[i] ) return false;		break;
			case Tag.BIN:	foreach( i,t; data5.tupleof ) if( data5.tupleof[i] != e.data5.tupleof[i] ) return false;		break;
			case Tag.MEM:	foreach( i,t; data6.tupleof ) if( data6.tupleof[i] != e.data6.tupleof[i] ) return false;		break;
			case Tag.CALL:	foreach( i,t; data7.tupleof ) if( data7.tupleof[i] != e.data7.tupleof[i] ) return false;		break;
			case Tag.ESEQ:	foreach( i,t; data8.tupleof ) if( data8.tupleof[i] != e.data8.tupleof[i] ) return false;		break;
			}
			return true;
		}
		return false;
