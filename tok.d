...
/// 
class ToknizeException : Exception
{
	this(FilePos fpos, string msg){ super("ToknizError" ~ fpos.toString ~ ": " ~ msg); }
}
...
//	PeekStream input;
	...
	bool input_skip_whitespace(){
		...
		while( !input.eof ){
			auto c = input.top;
			if( c!=' ' && c!='\t' && c!='\v' && c!='\f' ) break;	//改行はSkipしない
			input.forward();
			result = true;
		}
		return result;
	...
	body{
		...
		if( c == '0' ){
			...
			if( c == 'x' || c == 'X' ){								//16進数(0x??... | 0X??...)
				if( !isxdigit(c = tolower(input_next())) ) error("invalid hex literal.");
				do{
					i = (i * 16) + (isalpha(c) ? c-'a'+10 : c-'0');
					debug(Number) p("x=%X, %s", i, c);
				}while( isxdigit(c = tolower(input_next())) )
			}else if( c == 'b' || c == 'B' ){						//2進数(0b??... | 0B??...)
				if( !isbdigit(c = input_next()) ) error("invalid binary literal.");
				do{
					i = (i * 2) + (c - '0');
				...
				if( c == 'o' ){
					if( !isodigit(c = input_next()) ) error("invalid octet literal.");
				}
				do{
					i = (i * 8) + (c - '0');
...
//					assert(input.top == '.');
	...
	body{
		...
		while( (c = input.top) != '\"' ){
			...
			input.forward();
			if( c == '\\' ){ esc = true; continue; }	//エスケープフラグON
			
			if( len >= vbuf.length ){	// バッファの拡張
				vbuf.length = vbuf.length * 2;
			}
			if( esc ) c = esc_char(c), esc = false;
			vbuf[len++] = c;
		}
